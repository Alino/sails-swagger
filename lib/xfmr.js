...
const Transformer = {
    ...
    getDefinitions(sails) {
        ...
        definitions = Object.assign(definitions, Transformer.getDefinitionsFromSwaggerConfig(sails))
    ...
    getDefinitionsFromSwaggerConfig(sails) {
        let definitions = {}
        const swaggerConfigDefinitions = sails.config.swagger.definitions
        if (swaggerConfigDefinitions) {
            Object.assign(definitions, swaggerConfigDefinitions)
        }
        return definitions
    },
    ...
    getDefinitionsFromRouteConfig(sails) {
        ...
        let swaggerDefinitions = _.chain(routes)
            .pick(function(routeConfig, route) {
                ...
                var methods = swaggerdef.methods || [key.split(' ')[0]];
    ...
    getModelFromPath(sails, path) {
        ...
        if (childAttributeName) {
            let childAttribute = _.get(parentModel, ['attributes', childAttributeName]) || _.get(parentModel, ['attributes', pluralize.plural(childAttributeName)])
